...
test("Agenda - filtro: mostra apenas aulas Realizadas", () => {
  ...
  // --- dados fake ---
  const students = [{ id:"s1", name:"João" }];
  const base = new Date();
  const inDays = (d)=> {
    const x = new Date(base);
    x.setDate(x.getDate()+d);
    return `${x.getFullYear()}-${pad2(x.getMonth()+1)}-${pad2(x.getDate())}T18:30`;
  };

  const lessons = [
    { id:"l1", studentId:"s1", date: inDays(1),  status: 2, style:"Samba", place:"Sala A", price:120 }, // ✅ Realizada
    { id:"l2", studentId:"s1", date: inDays(2),  status: 1, style:"Samba", place:"Sala A", price:120 }, // Confirmada
    { id:"l3", studentId:"s1", date: inDays(45), status: 2, style:"Samba", place:"Sala A", price:120 }, // fora do range (30 dias)
  ];
  ...
  expect(rows[0].textContent).toContain("Realizada");
});
